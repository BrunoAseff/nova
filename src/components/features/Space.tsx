  ...
  return (
    <AutoSaveProvider userId={session?.user?.id}>
      <TooltipProvider>
        ...
        >
          <TabsList className="absolute bottom-6 left-8 z-10 min-h-[2.75rem] border-[2px] border-accent md:bottom-10 md:left-auto md:right-28">
            {[...spaces]
              .sort((a, b) => a.id - b.id)
              .map((space) => (
                <TabsTrigger
        ...
        <Tooltip delayDuration={0}>
          <TooltipTrigger asChild>
            <Button
              ...
              className="show-in-fullscreen absolute bottom-10 right-14 z-10 hidden overflow-hidden rounded-xl border-[2px] border-accent bg-background p-5 text-sm text-muted-foreground shadow-md animate-in fade-in-0 hover:bg-background hover:text-foreground md:flex md:p-5"
            >
              <AnimatedConfig />
            </Button>
          </TooltipTrigger>
